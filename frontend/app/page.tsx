...
export default function DashboardPage() {
  ...
  const tier = reputation ? Number((reputation as unknown[])[3]) : 0;
  const points = reputation ? Number((reputation as unknown[])[0]) : 0;

  return (
    <div className="space-y-6 stagger">
      <PageHeader title="Dashboard" subtitle="Welcome back to FlexLiving DAO" icon="◈" />
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon="🪙" label="FLEX Balance" value={formatFLEX(flexBalance as bigint)} gradient="from-violet-500 to-fuchsia-500" />
        <StatCard icon="💵" label="USDC Balance" value={`$${formatUSDC(usdcBalance as bigint)}`} gradient="from-emerald-400 to-teal-400" />
        <StatCard icon="🏘️" label="Active Properties" value={`${activeProperties?.toString() ?? "0"} / ${totalProperties?.toString() ?? "0"}`} gradient="from-blue-400 to-indigo-500" />
        <StatCard icon={TIER_EMOJIS[tier] ?? "🥉"} label="Reputation" value={hasSBT ? `${TIER_NAMES[tier]} · ${points} pts` : "No SBT"} gradient="from-amber-400 to-orange-500" />
      </div>
          ...
          <Card title="FLEX Tokenomics" icon="📊">
            <div className="space-y-5">
              <div className="flex justify-between items-baseline text-sm">
                ...
                <span className="text-white/80 font-mono text-xs">{formatFLEX(flexSupply as bigint)} / 100M</span>
              </div>
              <div className="w-full bg-white/[0.04] rounded-full h-1.5 overflow-hidden">
                <div className="bg-linear-to-r from-violet-500 to-fuchsia-500 h-full rounded-full transition-all duration-1000" style={{ width: `${flexSupply ? Math.max((Number(flexSupply) / 1e26) * 100, 2) : 2}%` }} />
              </div>
              <div className="grid grid-cols-4 gap-3">
                {[
                  ...
                  <div key={r.label} className="text-center p-3 rounded-xl bg-white/[0.03] border border-white/[0.04]">
        ...
        <div className="lg:col-span-2">
          <Card title="Quick Actions" icon="⚡">
            <div className="space-y-2">
              {[
                { href: "/properties", icon: "⬡", title: "Properties", desc: "Browse spaces", color: "group-hover:text-blue-400" },
                { href: "/escrow", icon: "◇", title: "Pay Rent", desc: "Earn 50 FLEX", color: "group-hover:text-emerald-400" },
                { href: "/reviews", icon: "☆", title: "Reviews", desc: "Earn 10 FLEX", color: "group-hover:text-amber-400" },
                { href: "/reputation", icon: "◎", title: "Reputation", desc: "View SBT tier", color: "group-hover:text-violet-400" },
                { href: "/governance", icon: "△", title: "Governance", desc: "Vote on proposals", color: "group-hover:text-fuchsia-400" },
              ].map((a) => (
                <Link key={a.href} href={a.href} className="flex items-center gap-3.5 p-3 rounded-xl hover:bg-white/[0.04] transition-all duration-300 group">
                  <span className={`text-sm w-8 h-8 rounded-lg bg-white/[0.04] flex items-center justify-center text-white/30 ${a.color} transition-colors`}>{a.icon}</span>
                  <div className="flex-1 min-w-0">
                    <p className={`text-[13px] font-medium text-white/70 ${a.color} transition-colors`}>{a.title}</p>
                    <p className="text-[11px] text-white/25">{a.desc}</p>
                  </div>
                  <svg className="w-3.5 h-3.5 text-white/10 group-hover:text-white/30 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                </Link>
              ))}
            </div>
          </Card>
      ...
      <Card title="Smart Contract Architecture" icon="🏗️">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 stagger">
          {[
            { name: "FlexToken", icon: "🪙", desc: "ERC20 + Votes", color: "from-fuchsia-500/10 to-violet-500/10 border-fuchsia-500/10" },
            { name: "PropertyNFT", icon: "🏠", desc: "ERC721 Listings", color: "from-blue-500/10 to-cyan-500/10 border-blue-500/10" },
            { name: "RentEscrow", icon: "💰", desc: "DeFi Payments", color: "from-emerald-500/10 to-teal-500/10 border-emerald-500/10" },
            { name: "ReviewSystem", icon: "⭐", desc: "AI Sentiment", color: "from-amber-500/10 to-orange-500/10 border-amber-500/10" },
            { name: "ReputationSBT", icon: "🎖️", desc: "Soulbound", color: "from-violet-500/10 to-purple-500/10 border-violet-500/10" },
            { name: "FlexGovernor", icon: "🗳️", desc: "DAO Gov", color: "from-pink-500/10 to-rose-500/10 border-pink-500/10" },
          ].map((c) => (
            <div key={c.name} className={`text-center p-4 rounded-xl bg-linear-to-b ${c.color} border transition-all duration-300 hover:scale-[1.03]`}>
  ...
  return (
    <div className="-mx-4 sm:-mx-6 lg:-mx-8 -mt-8">
      ...
      <section className="relative min-h-[92vh] flex items-center justify-center overflow-hidden px-4">
        ...
        <div className="orb w-[500px] h-[500px] bg-fuchsia-600 -bottom-32 -right-32" style={{ animationDelay: "-7s" }} />
        <div className="orb w-[300px] h-[300px] bg-blue-600 top-1/3 right-1/4" style={{ animationDelay: "-12s" }} />
        ...
        <div className="relative text-center max-w-4xl mx-auto z-10">
          ...
          <div className="animate-fadeUp inline-flex items-center gap-2.5 bg-white/[0.04] border border-white/[0.06] rounded-full px-5 py-2 mb-10 backdrop-blur-sm">
            ...
            <span className="text-[12px] text-white/50 tracking-wider uppercase font-medium">Live on Polkadot Hub</span>
          ...
          <p className="animate-fadeUp delay-200 text-lg sm:text-xl text-white/30 max-w-2xl mx-auto mb-6 leading-relaxed font-light">
            The <span className="text-white/60">decentralized co-living protocol</span> with
            trustless rent payments, AI-verified reviews, and soulbound reputation —
            all governed by its community.
          </p>
      ...
      <section className="max-w-6xl mx-auto px-4 py-24">
        <div className="text-center mb-16 animate-fadeUp">
          <p className="text-[12px] text-violet-400/60 tracking-widest uppercase mb-3 font-medium">Protocol Features</p>
          ...
          {features.map((f, i) => (
            ...
            >
              <div className="flex items-start justify-between mb-5">
                <div className={`w-12 h-12 rounded-2xl bg-linear-to-br ${f.gradient} flex items-center justify-center text-xl opacity-80 group-hover:opacity-100 transition-opacity shadow-lg`}>
                  {f.icon}
                </div>
              ...
              <h3 className={`text-lg font-bold bg-linear-to-r ${f.gradient} bg-clip-text text-transparent mb-2`}>
                {f.title}
              </h3>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-24">
        <div className="text-center mb-12 animate-fadeUp">
          <p className="text-[12px] text-fuchsia-400/60 tracking-widest uppercase mb-3 font-medium">Architecture</p>
          <h2 className="text-3xl sm:text-4xl font-bold text-white/90 tracking-tight">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 stagger">
          {contracts.map((c) => (
            ...
            >
              <span className="text-2xl block mb-3 group-hover:scale-110 transition-transform duration-300">{c.icon}</span>
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-24 text-center">
          <div className="animate-fadeUp">
            ...
            <p className="text-white/25 text-sm mb-10 max-w-md mx-auto">
              Connect your wallet to explore properties, pay rent with crypto, build your soulbound reputation, and govern the platform.
            </p>
